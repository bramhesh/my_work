...
//function to check if pokemon is next evolution of some other pokemon
let pne = (name) =>{

  //letiable to check if pokemon found
  let pokemonFound = false

  //iterating all pokemons
  for (let pokeCount = 0; pokeCount < pokedex.pokemon.length; pokeCount++) {

    //checking if the next evolution is not empty or null
    if(pokedex.pokemon[pokeCount].next_evolution!=undefined){

      //iterating all the next evolutions of a program
      for (let pokeEvolution = 0; pokeEvolution < pokedex.pokemon[pokeCount].next_evolution.length; pokeEvolution++) {

        //checking if the name entered is soem other pokemon's next evolution
        if (name==pokedex.pokemon[pokeCount].next_evolution[pokeEvolution].name) {
          pokemonFound = true

          //displaying name of pokemon
    ...
    //if pokemon is not any other pokemon's next evolution
    if(pokemonFound == false){
      alert("Enter correct pokemon name")
    }
}
//asking user to enter the pokemon's name
let PokemonName = window.prompt('Enter the pokemon name')
let pName = PokemonName.charAt(0).toUpperCase() + PokemonName.slice(1)
//passing user's input to the function
pne(pName)