...
//function declaration which checks for weakness of a pokemon and displays the name of tha pokemon
let Weakness = (weaknessOfPokemon) =>{

//weaknessFound declared for checking if correct weakness is entered by user
let weaknessFound = false

//for loop for iterating all the pokemons
  for (let pokeCount = 0; pokeCount < pokedex.pokemon.length; pokeCount++) {

    //another for loop for iterating all weaknesses of each pokemon
    for (let pokeWeakness = 0; pokeWeakness < pokedex.pokemon[pokeCount].weaknesses.length; pokeWeakness++) {
    
      //checking if the weakness is equal to the weakness entered by user 
     if (pokedex.pokemon[pokeCount].weaknesses[pokeWeakness]==weaknessOfPokemon) {
        weaknessFound = true

        //alerting the name of the pokemon whose weaknesses matches
		    alert(pokedex.pokemon[pokeCount].name)
        break
	   }
    }
  }
  //if the weakness is not found
    if(weaknessFound==false){
      alert("Enter Correct weakness")
    }
}

//asking user to enter the weakness
let weaknessName = window.prompt('Enter the weakness of pokemon')
//Converting the first letter of the input string to upper case
let str = weaknessName.charAt(0).toUpperCase() + weaknessName.slice(1)
//passing user's input to function
Weakness(str)
