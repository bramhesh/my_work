...
//function providing the info of a pokemon
let pokeInfo = (name) =>{

  //pokemonFound to check if correct pokemon name is entered by user
  let pokemonFound = false

  //loop for iterating all the pokemons
  for (let pokeCount = 0; pokeCount < pokedex.pokemon.length; pokeCount++) {

    //check if the pokemon matches with the entered name
    if(pokedex.pokemon[pokeCount].name==name){
      pokemonFound = true

      /*Since the result cannot be displayed by alert
      The pokemon data is displayed in the console*/
      alert("The data is displayed in the console log")
      console.log(pokedex.pokemon[pokeCount])
    }
  }

    //pokemon not found
    if (pokemonFound == false) {
      alert("Pokemon doesn't exist. Please enter correct pokemon name")
    }
}
...
//passing the name entered to the function
pokeInfo(pName)
